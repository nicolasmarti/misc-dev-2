...
let test_kind k = 
  Format.eprintf "input=%S@." k;
  let stream = Stream.from_string ~filename:"stdin" k in
  match parse_kind stream with
  | Result.Ok (res, _) -> 
      printbox (token2box (kind2token res) 400 2)
  | Result.Error (pos, k) ->
      Format.eprintf "%a: syntax error: %s@." Position.File.format pos k;
      raise Exit
;;
...
let test_mltype k = 
  Format.eprintf "input=%S@." k;
  let stream = Stream.from_string ~filename:"stdin" k in
  match parse_mltype stream with
  | Result.Ok (res, _) -> 
      printbox (token2box (mltype2token res) 400 2)
  | Result.Error (pos, k) ->
      Format.eprintf "%a: syntax error: %s@." Position.File.format pos k;
      raise Exit
;;
  ...
  test_term
       "let rec x := y; 
        ...
        in 
           (\\ (Just doudou) -> 
                  case doudou of 
                        | petit caca with gros prout := redoudou
                        | gros caca with petit prout := redoudou
                                    with gros prout := if pas odeur then ca ira else pas du tout roudoudou

           ) x"

let _ = test_kind "* -> (* -> *) -> *"

let _ = test_mltype "(a -> b) -> list a -> list b"
