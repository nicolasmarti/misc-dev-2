...
class Strat(Subscriber):
    ...
    def __init__(self, ticker, exchg, cur):
        ...
        self.opentimeout = 10
        self.closetimeout = 10
        self.stoplossN = 0.008
        self.stopgainN = 0.008
        self.size = 100
    ...
    def event(self, event):
        ...
        if fieldname == "LAST":
            ...
            if l > self.barperiod:
                self.lastbar = time()            
                self.mts.addData([quote,quote,quote,quote, None, None])
            ...
            if mu <> None and sig <> None:
                ...
                if self.position == "OPENED":
                    ...
                    if res <> -1:
                        ...
                        if pos == 0:
                            ...
                            if (avgprice - self.mts.getData()[0][3])/avgprice > self.stoplossN:
                                print (avgprice, self.mts.getData()[0][3])
                                self.position = "CLOSED"
                                print "OPENED -> CLOSED (stoploss)"
                            ...
                            elif self.mts.getData()[0][3] >= mu + (self.K2 * sig) or (self.mts.getData()[0][3] - avgprice)/avgprice > self.stopgainN: 
                                                            
                                print (avgprice, self.mts.getData()[0][3])
                                self.position = "CLOSED"
                                print "OPENED -> CLOSED (stopgain)"
                                self.close.append((len(self.mts.getData()), self.mts.getData()[0][3], "stopgain"))
                                self.oidc = o.closelmtOrder(self.oid, self.mts.getData()[0][3])        
                                self.closetime = time()
                ...
                if self.position == "CLOSED":                    
                    ...
                    if res <> -1 and res[1] == 0:                            
                        if time() - self.closetime > self.closetimeout:
                            o.closeOrder(self.oidc)        
                            self.close.append((len(self.mts.getData()), self.mts.getData()[0][3], "timeout(close)"))
                            self.oidc = o.closelmtOrder(self.oid, self.mts.getData()[0][3])        
                            self.closetime = time()
                    else:
                        if self.mts.getData()[0][3] <= mu - (self.K * sig):                            
                            if len(self.mts.getData()) > 3 + self.N and self.mts.getData()[2][4] - (self.K * self.mts.getData()[2][5]) - 0.2 <= self.mts.getData()[1][4] - (self.K * self.mts.getData()[1][5]) and self.mts.getData()[1][4] - (self.K * self.mts.getData()[1][5]) - 0.2 <= self.mts.getData()[0][4] - (self.K * self.mts.getData()[0][5]):
                                self.position = "OPENED"
                                print "CLOSED -> OPENED(long)"
