...
type 'a objdef = 
  | DefNode of ('a objdef) VarMap.t
  | DefLeaf of 'a VarMap.t
;;


let rec defname2path name =  
  try
...
let rec insertpathdef name elt d = 
  match name with
    | [] -> None
    | x::[] -> (
...
let rec mkdefinitionfromdef path def =
  try
