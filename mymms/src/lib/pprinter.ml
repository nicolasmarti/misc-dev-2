...
let addtopspace (n: int) (b: box) : box =
    {
      data = (list_of_n_elt (String.concat "" (list_of_n_elt " " b.width)) n) @ b.data;
      height = b.height + n;
      width = b.width;
    }
;;
...
let addbottomspace (n: int) (b: box) : box =
    {
      data = b.data @ (list_of_n_elt (String.concat "" (list_of_n_elt " " b.width)) n);
      height = b.height + n;
      width = b.width;
    }
;;
...
let string_to_box s = 
  ...
  let l' = List.map (fun hd -> if (String.length hd < lmax) then String.concat "" (hd :: (list_of_n_elt " " (lmax - String.length hd ))) else hd) l in
    { data = l';
...
let rec token2box (t: token) (w: int) (indent: int) : box =
  match t with
	...
	let spacebox n = { data = (String.concat "" (list_of_n_elt " " n))::[];
			   height = 1;
			   width = n;
			 } in
