...
getGlobalModules :: ModulePath -> TypeM [TCEnv]                       
getGlobalModules path = do {
    ; globals <- getGlobals
    ; case Map.lookup path (moduleAlias globals) of
        Nothing -> throwError $ ErroNoModule path
        Just aliaspathes -> Set.fold (\ hd acc -> getModule hd (moduleTree globals) : acc) [] aliaspathes
    }
...
getDefinition [name] = do {
    ; env <- getEnv
    ; others <- getGlobalModules 
    ; listsdef <- foldM (\ hd acc -> do {
                             ; catchError (do {
                                               ; def <- getTCEnvDefinition name hd
                                               ; return $ def:acc
                                               }
                                          ) (\ (ErrNoDef _) -> return acc)
                             }
                        ) [] $ env:others
    ; error ""
    }
...
reduceTerm te config = reduce' te
    where
        ...
        -- the typechecking has not been done -> take the first def that matches
        reduce' te@(Operator op s pos ty Nothing) | delta config = do {
            ; def <- getDefinition ["(" ++ s ++ ")"]
            ; return $ Operator op s pos ty $ Just def
            }
                                                  | otherwise = return te
