...
class Strat(Subscriber):
    ...
    def __init__(self, ticker, exchg, cur):
        ...
        self.stopgainN = 0.01
        self.mingainN = 30.0/self.size
        self.maxlossN = 60.0/self.size
                    ...
                    if res <> -1:
                        ...
                        if pos == 0:
                            ...
                            # if a reverse marteau in 
                                ...
                                print (avgprice, self.mts.getData()[0][3])
                                self.position = "CLOSED"
                                print "OPENED -> CLOSED (stopuptrend)"
                                self.close.append((len(self.mts.getData()), self.mts.getData()[0][3], "stopuptrend"))
                                self.oidc = o.closelmtOrder(self.oid, self.mts.getData()[0][3])        
                                self.closetime = time()
                ...
                if self.position == "CLOSED":                    
                    ...
                    if res <> -1 and res[1] == 0:                            
                        ...
                        if self.mts.getData()[0][3] <= mu - (self.K * sig) and len(self.mts.getData()) > ls + self.N:                            
                            test = True
                            for i in range(0,ls):
                                test = test and (self.mts.getData()[i][4] - ((self.K) * self.mts.getData()[i][5]) >= self.mts.getData()[i+1][4] - (self.K * self.mts.getData()[i+1][5]))
                            if test:
