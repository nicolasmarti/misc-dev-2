...
let pure_resolution fm =
  resloop([],filter (non trivial) (simpcnf(specialize(pnf fm))));;
...
let pure_presolution fm =
  presloop([],filter (non trivial) (simpcnf(specialize(pnf fm))));;
...
let pure_resolution fm =
  let cls = filter (non trivial) (simpcnf(specialize(pnf fm))) in
