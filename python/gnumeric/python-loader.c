...
gplp_load_base (GOPluginLoader *loader, GOErrorInfo **ret_error)
{
	...
	gnm_py_interpreter_load(py_interpreter_info);
	...
	if (full_module_file_name == NULL) {
		*ret_error = go_error_info_new_printf (
		...
		gnm_py_interpreter_save(py_interpreter_info);
			      ...
			      GOErrorInfo **ret_error)
{
	...
	printf("gplp_load_service_file_saver(start)\n");
...
call_python_function_args (GnmFuncEvalInfo *ei, GnmValue const * const *args)
{
	...
	printf("call_python_function_args(start)\n");
	...
	SWITCH_TO_PLUGIN (go_plugin_service_get_plugin (service));
	gnm_py_interpreter_load(PLUGIN_GET_LOADER (go_plugin_service_get_plugin (service))->py_interpreter_info);
	...
	GnmValue* retval = call_python_function (python_fn, ei->pos, n_args, args);

	gnm_py_interpreter_save(PLUGIN_GET_LOADER (go_plugin_service_get_plugin (service))->py_interpreter_info);

	printf("call_python_function_args(end)\n");
